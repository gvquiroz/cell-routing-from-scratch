...
**Random shuffle**: Each tenant's shard is a random subset of size `k` from a pool of `N` cells. Randomness ensures even distribution, so no two tenants have identical shards unless by chance.
...
Shuffle sharding **requires** health-aware routing. Without health checks, router can't distinguish healthy cells from failed cells, and shuffle sharding degrades to random selection.
...
**Failover order**: Within a shard, router tries cells in deterministic order (e.g., sorted by cell ID). This avoids thundering herd because all tenants with same shard fail over to same cells in same order.
