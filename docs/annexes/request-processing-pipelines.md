...
**State mutation**: Stages that modify request context (add headers, rewrite URL) can create unintended coupling. Stage 5 might depend on header added by stage 2, creating an implicit dependency. Immutable context between stages prevents this but adds overhead (copy-on-write).
